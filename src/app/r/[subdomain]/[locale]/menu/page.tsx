...
export default function SubdomainMenuPage() {
  ...
  useEffect(() => {
    refreshCartSummary();
    document.addEventListener("visibilitychange", refreshCartSummary);
    return () => document.removeEventListener("visibilitychange", refreshCartSummary);
  }, [refreshCartSummary]);
  ...
  return (
    <div className="min-h-screen bg-stone-50 pb-24">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {filteredCategories.map((cat) => (
                ...
                >
                  ...
                  <span className="text-xs text-stone-500">{cat.menuItems.length} ta</span>
                </a>
              ))}
            </div>
          </section>
      ...
      >
        <span className="font-medium">Savat {cartCount > 0 ? `(${cartCount})` : ""}</span>
